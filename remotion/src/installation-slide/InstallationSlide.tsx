...
import type {
    ...
    InstallationChecklistScene,
    InstallationScene,
    InstallationSlideProps,
    InstallationVideoScene,
} from "./types";
...
const getSceneStart = (scenes: InstallationScene[], index: number): number => {
    return scenes
        .slice(0, index)
        .reduce((total, scene) => total + scene.durationInFrames, 0);
};

const getChecklistItemLabel = (
...
const ChecklistSceneView: React.FC<{
    items: InstallationChecklistItem[];
    scene: InstallationChecklistScene;
    language: "en" | "ja";
}> = ({ items, scene, language }) => {
    const frame = useCurrentFrame();
    const { fps } = useVideoConfig();
    ...
    return (
        <AbsoluteFill>
            {content.scenes.map((scene, index) => {
                const start = getSceneStart(content.scenes, index);
                const sceneTitle = getChecklistItemLabel(
