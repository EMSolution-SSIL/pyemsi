...
def test_build_run_command_executable(tmp_path):
    input_path = str(tmp_path / "transient.json")
    exe_path = str(tmp_path / "EMSolution.exe")

    command = build_run_command(input_path, backend="executable", executable_path=exe_path)
...
def test_build_run_command_executable_requires_executable_path(tmp_path):
    ...
    try:
        build_run_command(input_path, backend="executable", executable_path=None)
...
def test_build_run_command_rejects_unknown_backend(tmp_path):
    ...
    try:
        build_run_command(input_path, backend="bogus")
        assert False, "expected ValueError"
    except ValueError:
        pass
